...
pub struct GPUVectorU32 {
    // Our initial, cpu-side data
    pub cpu_data: Vec<u32>,
...
impl GPUVectorU32 {
    pub fn new(handles: &GPUHandles, cpu_data: Vec<u32>, label: &str, ouput_buffer: bool) -> Self {
        let element_size: usize = std::mem::size_of::<u32>();
        ...
        GPUVectorU32 { cpu_data, staging_buffer, storage_buffer }
    }
    ...
    pub fn transfer_from_gpu_to_cpu_mut(&mut self, encoder: &mut CommandEncoder) {
        ...
        if self.staging_buffer.is_some() {
            encoder.copy_buffer_to_buffer(
                ...
                0,
                (self.cpu_data.len() * mem::size_of::<u32>()) as u64,
            );
        }
    }
}
