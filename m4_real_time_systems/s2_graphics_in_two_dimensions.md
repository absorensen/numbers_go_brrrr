...
format such as JPEG, we will also become very aware of unnatural compression artifacts. Lines on the
...
The point primitive is sometimes available. It will usually draw a quad with a size you have to set per vertex. The
...
[Painter's algorithm][12]. When we aren't doing transparency, the reverse is more efficient. Writes are
...
isn't part of rendering. In face, in order to figure out which element was clicked by a mouse, we might instead
...
on a canvas and representing each line as a circle. Every single frame we would get in a signal of where the
...
Alot of this moderately advanced rendering stuff is actually sort of redundant.
...
websites [LearnWGPU][0] and [LearnOpenGL][1] for one thousandth time.
